...
class YahooStatServiceImplTest {
  ...
  void testGetAllTeamsStats() throws IOException {
    ...
    String mockFantasyResponseScoreboardPath = "src/test/resources/yahoo/fantasy_response_scoreboard_mock.json";
    String mockAllTeamsStatsPath = "src/test/resources/yahoo/all_teams_stats_mock.json";
    List<StatCategory> relevantCategories = objectMapper.readValue(new File(mockRelevantCategoriesPath),
        new TypeReference<>() {
        });

    FantasyResponseDTO response = objectMapper.readValue(
    ...
    List<TeamStatCategory> allTeamsStats = yahooStatService.getAllTeamsStats(leagueId, week, relevantCategories);
